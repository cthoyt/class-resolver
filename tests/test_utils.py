...
class TestUtilities(unittest.TestCase):
    ...
    def test_get_subclasses(self) -> None:
        ...
        if sys.version_info < (3, 13):
            ...
            self.assertIn(
                enum._EnumDict,
                set(get_subclasses(dict, exclude_external=False, exclude_private=False)),
            )
            self.assertNotIn(
                enum._EnumDict,
                set(get_subclasses(dict, exclude_external=False, exclude_private=True)),
            )
            self.assertNotIn(
                enum._EnumDict,
                set(get_subclasses(dict, exclude_external=True, exclude_private=False)),
            )
            self.assertNotIn(
                enum._EnumDict,
                set(get_subclasses(dict, exclude_external=True, exclude_private=True)),
            )
